...
StatesGroup ImuProcess::imu_preintegration( const StatesGroup &state_in, std::deque< sensor_msgs::msg::Imu::ConstSharedPtr > &v_imu, double end_pose_dt )
{
    ...
    // g_lidar_star_tim,
    //        rclcpp::Time(v_imu.back()->header.stamp).seconds() - g_lidar_star_tim,
...
#ifdef DEBUG_PRINT
// fout<<rclcpp::Time(head->header.stamp).seconds()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
#endif
        dt = rclcpp::Time(tail->header.stamp).seconds() - rclcpp::Time(head->header.stamp).seconds();
        /* covariance propagation */
...
void ImuProcess::Process( const MeasureGroup &meas, StatesGroup &stat, PointCloudXYZINormal::Ptr cur_pcl_un_ )
{
    ...
    {
      std::cout << "ASSERTION (meas.lidar != nullptr) FAILED\n";
    }
    ...
    if ( imu_need_init_ )
    {
        ...
        if ( init_iter_num > MAX_INI_COUNT )
        {
            ...
            std::cout << "IMU Initials: Gravity: " << stat.gravity[ 0 ] << " " << stat.gravity[ 1 ] << " " << stat.gravity[ 2 ]
                      << "; state.bias_g: " << stat.bias_g[ 0 ] << " " << stat.bias_g[ 1 ] << " " << stat.bias_g[ 2 ] << "; acc covarience: "
                      << cov_acc[ 0 ] << " " << cov_acc[ 1 ] << " " << cov_acc[ 2 ] << "; gry covarience: " << cov_gyr[ 0 ] << " " << cov_gyr[ 1 ]
                      << " " << cov_gyr[ 2 ] << std::endl;
        }
